    ...
    for (size_t i = 0; i < queueFamilies.size(); ++i)
    {
        ...
        if (!indices.presentFamily.has_value())
        {
            ...
            VkCheckResult(device.getSurfaceSupportKHR(i, surface, &supported),
                "[VULKAN] Failed querying surface support on physical device!");
            if (supported)
            {
...
void VulkanContext::InitializeDevice()
{
    ...
    vk::StructureChain<vk::DeviceCreateInfo, vk::PhysicalDeviceFeatures2, vk::PhysicalDeviceSynchronization2Features,
        vk::PhysicalDeviceBufferDeviceAddressFeatures, vk::PhysicalDeviceAccelerationStructureFeaturesKHR, vk::PhysicalDeviceRayTracingPipelineFeaturesKHR> structureChain;
    ...
    VkCheckResult(_physicalDevice.createDevice(&createInfo, nullptr, &_device), "[VULKAN] Failed creating a logical device!");
